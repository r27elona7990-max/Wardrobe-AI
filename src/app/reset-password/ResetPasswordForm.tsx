...
import { KeySquare, ArrowRight, Loader2 } from "lucide-react";
  ...
  if (!token) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-nebula-bg p-4 relative overflow-hidden">
        <div className="w-full max-w-md space-y-8 glass p-10 rounded-nebula relative z-10 border border-black/10 shadow-2xl text-center">
          <h1 className="text-2xl font-black text-red-500">Invalid Link</h1>
          <p className="text-sm text-nebula-on-surface/60">The password reset link is missing a valid security token.</p>
          <Link href="/forgot-password" className="inline-block mt-4 text-nebula-secondary font-bold hover:underline">
            Request a new link
          </Link>
        </div>
      </div>
      ...
      <div className="w-full max-w-md space-y-8 glass p-10 rounded-nebula relative z-10 border border-black/10 shadow-2xl">
        <div className="flex flex-col items-center space-y-4">
          ...
          <div className="text-center space-y-2">
            <h1 className="text-3xl font-black tracking-tighter">Create New Password</h1>
            <p className="text-sm text-nebula-on-surface/40">Enter a strong encryption key to secure your vault.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-nebula-on-surface/40 ml-1">New Password</label>
              <input 
                name="password"
                required
                type="password" 
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-nebula-on-surface/40 ml-1">Confirm Password</label>
              <input 
